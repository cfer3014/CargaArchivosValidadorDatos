...
function detectColumns(record) {
    ...
    Object.keys(record).forEach(original => {

        const normalized = original
            .toLowerCase()
            .trim();

        for (const standard in aliases) {

            if (
                aliases[standard].includes(normalized)
            ) {
...
function normalizeRecord(record, mapping) {
    ...
    return {
        ...
        cliente:
            get('cliente', 'Desconocido'),

        producto:
            get('producto', 'Sin producto'),
        ...
        fecha:
            get('fecha', new Date().toISOString()),
...
function analyzeData(records) {
    ...
    const mapping = detectColumns(records[0]);

    const normalizedRecords = records.map(
        r => normalizeRecord(r, mapping)
    );
    ...
    normalizedRecords.forEach(record => {
        ...
        if (
            !record.tipo.includes('compra') &&
            !record.tipo.includes('purchase') &&
            !record.tipo.includes('venta')
        ) {
            return;
        }
    ...
    Object.values(clientStats)
        .forEach(client => {
            ...
            client.productos =
                Array.from(client.productos);

            let segment = 'Bajo';

            if (client.montoTotal >= 1000)
                segment = 'Premium';

            else if (client.montoTotal >= 500)
                segment = 'Alto';

            else if (client.montoTotal >= 100)
                segment = 'Medio';
